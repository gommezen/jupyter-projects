...
df = yf.download('PLTR', period='1y')

# Download multiple tickers
df = yf.download(['PLTR', 'MSFT', 'GOOGL'], period='1y')
```
...
# Year to date
df = yf.download('PLTR', period='ytd')

# All available historical data
df = yf.download('PLTR', period='max')
```

### `start` and `end` - Custom Date Ranges
```python
# Specific date range
df = yf.download('PLTR', start='2023-01-01', end='2025-11-07')
```
...
```python
# Daily data (default)
df = yf.download('PLTR', period='1y', interval='1d')

# Weekly data
df = yf.download('PLTR', period='1y', interval='1wk')

# Hourly data (max 730 days)
df = yf.download('PLTR', period='1mo', interval='1h')
```
...
```python
df = yf.download(['PLTR', 'MSFT'], period='1y')

# Access specific ticker's Close price
PLTR_close = df['Close']['PLTR']
...
# or
close_price = df[('Close', 'PLTR')]  # If MultiIndex
```
...
ax.set_title('PLTR Stock Price - 1 Year', fontsize=16, fontweight='bold')
...
- **Tech:** PLTR (Palentir), MSFT (Microsoft), GOOGL (Google), AMZN (Amazon), META (Meta)
